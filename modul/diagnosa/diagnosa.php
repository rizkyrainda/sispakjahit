<title>Diagnosa - Sispak Jahit</title>
<?php
switch ($_GET['act']) {
  ...
  default:
    if ($_POST['submit']) {
      ...
      $sqlpkt = mysql_query("SELECT * FROM kerusakan order by kode_kerusakan+0");
...
// --------------------- START ------------------------
      $sqlkerusakan = mysql_query("SELECT * FROM kerusakan order by kode_kerusakan");
      $arkerusakan = array();
      while ($rkerusakan = mysql_fetch_array($sqlkerusakan)) {
        $cftotal_temp = 0;
        $cf = 0;
        $sqlgejala = mysql_query("SELECT * FROM basis_pengetahuan where kode_kerusakan=$rkerusakan[kode_kerusakan]");
        $cflama = 0;
        ...
        if ($cflama > 0) {
          $arkerusakan += array($rkerusakan[kode_kerusakan] => number_format($cflama, 4));
        }
      }
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      echo "<div class='callout callout-default'>Jenis kerusakan sparepart yang rusak adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2) . " % (" . $vlpkt[1] . ")<br></h3>";
	 ...
	 <div class='alert alert-success alert-dismissible'>
                ...
                Silahkan memilih gejala sesuai dengan kondisi mesin anda, anda dapat memilih kepastian kondisi mesin dari pasti tidak sampai pasti ya, jika sudah tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
