<title>Pengetahuan - Sispak Jahit</title>
<?php
...
switch($_GET[act]){
  ...
  $tampil=mysql_query("SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan");
	...
	if ($_POST[Go]){
			$numrows = mysql_num_rows(mysql_query("SELECT * FROM basis_pengetahuan b,kerusakan p where b.kode_kerusakan=p.kode_kerusakan AND p.nama_kerusakan like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysql_query("SELECT * FROM basis_pengetahuan b,kerusakan p where b.kode_kerusakan=p.kode_kerusakan AND p.nama_kerusakan like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
	...
	$sql = mysql_query("SELECT * FROM gejala where kode_gejala = '$r[kode_gejala]'");
	$rgejala=mysql_fetch_array($sql);
	$sql2 = mysql_query("SELECT * FROM kerusakan where kode_kerusakan = '$r[kode_kerusakan]'");
	$rkerusakan=mysql_fetch_array($sql2);
       echo "<tr class='".$warna."'>
			 <td align=center>$no</td>
  ...
  case "tambahpengetahuan":
	echo "	<div class='alert alert-success alert-dismissible'>
				...
				<b>Contoh:</b><br>
				Jika kepercayaan <b>(MB)</b> anda terhadap gejala Hasil jahitan Loncat-loncat untuk kerusakan Hook Tumpul adalah <b>0.8 (Hampir Pasti)</b><br>
				Dan ketidakpercayaan <b>(MD)</b> anda terhadap gejala Hasil jahitan Loncat-loncat untuk kerusakan Hook Tumpul adalah <b>0.2 (Hampir Mungkin)</b><br><br>
				<b>Maka:</b> CF(Pakar) = MB – MD (0.8 - 0.2) = <b>0.6</b> <br>
				Dimana nilai kepastian anda terhadap gejala jahitan Loncat-loncat untuk kerusakan Hook Tumpul adalah <b>0.6 (Kemungkinan Besar)</b>
              </div>
		...
		$hasil4 = mysql_query("SELECT * FROM kerusakan order by nama_kerusakan");
  ...
  case "editpengetahuan":
	...
	<form name=text_form method=POST action='$aksi?module=pengetahuan&act=update' onsubmit='return Blank_TextField_Validator()'>
		...
		$hasil4 = mysql_query("SELECT * FROM kerusakan order by nama_kerusakan");
		while($r4=mysql_fetch_array($hasil4)){
			echo "<option value='$r4[kode_kerusakan]'"; if($r[kode_kerusakan]==$r4[kode_kerusakan]) echo "selected";
			echo ">$r4[nama_kerusakan]</option>";
		}
		echo	"</select></td></tr>
